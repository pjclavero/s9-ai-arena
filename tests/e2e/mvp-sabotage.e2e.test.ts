...
describe("T12.1 · 6 sabotajes deliberados, uno por paso del criterio 26.1", () => {
  ...
  it("sabotaje 4 (espectador): tickets falsificados o reutilizados no abren el canal", async () => {
    const gateway = new SpectateGateway({ port: 0, publicSpectateEnabled: true });
    ...
    const closeCode = (ticket: string) =>
      new Promise<number>((resolve, reject) => {
        const ws = new WebSocket(`ws://127.0.0.1:${gateway.port}/spectate/${battleId}`, [
          "spectate.v1",
          `ticket.${ticket}`,
        ]);
    ...
    try {
      ...
      const first = new WebSocket(`ws://127.0.0.1:${gateway.port}/spectate/${battleId}`, [
        "spectate.v1",
