...
function connect(): void {
  ...
  socket.on("open", () => socket.send(JSON.stringify({
    ...
    botId: process.env.BOT_ID ?? "bot-red", name: process.env.BOT_NAME ?? "Red Runner", team: "red"
  })));
  socket.on("message", (raw) => {
    ...
    const command: BotCommand = {
      ...
      movement: { throttle: 0.75, turn: 0.35 },
      turret: { turn: angleTurn(observation.self.turretHeading, desired), fire: Boolean(enemy) }
    };
