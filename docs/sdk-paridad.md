...
| Simulador local | `LocalSimulator`: lanza `apps/arena-engine/src/local-sim.ts` como **subproceso** Node (necesita `npx` en el `PATH`) y conecta los bots Python por WebSocket a `localhost`. | `startLocalBattle()` (en `src/local-simulator.ts`; `tests/helpers.ts` es un envoltorio con los defaults rápidos de test): importa `Battle`/`ProtocolServer` **directamente**, en el mismo proceso — no hace falta un subproceso porque el bot de JS y el motor ya corren en el mismo runtime (Node). | Real y notable: el SDK de JS puede probarse sin volver a arrancar Node; el de Python siempre paga el coste de un `subprocess.Popen` (unas décimas de segundo). A cambio, el SDK de Python es el único que demuestra de verdad que el protocolo cruza un límite de proceso (y de lenguaje) real, que es más parecido a cómo correrá un bot en producción. |
| CLI de un solo bot | `arena-sim <bot.py> --archetype ... --opponent idle` (instalado como *entry point* de `pip`) | `arena-sim <bot.ts\|js> --archetype ... --opponent idle` (`src/arena-sim.ts`, instalado como `bin` del workspace: `npx arena-sim ...` desde la raíz del repo; cubierto en R2.8, era la carencia declarada en E5) | Cubierto. Mismos flags (`--archetype`, `--opponent`, `--opponent-archetype`, `--map`, `--ruleset`, `--ticks`, `--seed`) y misma salida (el `BattleResult` en JSON por stdout). Diferencias honestas que quedan: (1) el de JS corre motor y bot **en el mismo proceso** (reutiliza `src/local-simulator.ts`), el de Python lanza el motor como subproceso — coherente con la fila "Simulador local"; (2) el de JS acepta además `--tick-interval-ms` para acelerar el bucle (el de Python solo corre a ritmo real, ~33 ms/tick); (3) el de Python se instala con `pip install -e .` en cualquier entorno, el de JS solo existe dentro del monorepo (`@arena/sdk` es `private`, no se publica en npm). |
