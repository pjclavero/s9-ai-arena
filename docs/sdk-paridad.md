...
| CLI de un solo bot | `arena-sim <bot.py> --archetype ... --opponent idle` (instalado como *entry point* de `pip`) | No se ha construido un CLI equivalente en esta entrega | Real, es una carencia: nada en el diseño lo impide (el helper `startLocalBattle` ya hace el trabajo pesado), simplemente no se ha escrito el envoltorio de CLI. Ver "Lo que queda fuera" en `docs/entrega-E5.md`. |
