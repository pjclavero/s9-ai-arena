...
export function diffDrafts(base: LoadoutDraft, draft: LoadoutDraft): string[] {
  ...
  for (const [slot, m] of draftBySlot) {
    ...
    else if ((prev.ammo ?? "") !== (m.ammo ?? ""))
      changes.push(`${slot} munición: ${prev.ammo ?? "—"} → ${m.ammo ?? "—"}`);
  }
...
function slotsFromDraft(d: LoadoutDraft | undefined): {
  ...
  if (chassisOptions.length === 0) {
    return (
      <div className="card" role="alert" data-testid="editor-catalog-error">
        ...
        <p className="error">
          El catálogo de módulos no está disponible o no contiene ningún chasis. Recarga o avisa al administrador.
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="card" role="alert" data-testid="editor-catalog-error">
        ...
        <button
          type="button"
          onClick={() => {
            setChassis(versioned(chassisOptions[0]));
            setSlots({});
            setAmmoBySlot({});
          }}
        >
