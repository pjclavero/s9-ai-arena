...
export const CRITERIA = [
  ...
  },
  {
    ...
    comando: ["npx", "vitest", "run", "apps/arena-engine/tests/robustness.test.ts", "-t", "presupuesto", "--maxWorkers=2"],
  ...
  },
  {
    ...
    cobertura: "parcial: las etapas containerizadas (protocol_test/smoke_battle/resource_limits y escape en contenedor real) requieren un runner con Docker (T6.2)",
  },
  {
    id: "mapas",
    ...
    evidencia: "query sobre map_versions publicadas + re-validación con el validador real de E4 + conversión a arena del motor",
    cobertura: "completa",
  },
  {
    id: "web",
    nombre: "Web: recuperación de conexión del visor y ausencia de información privada (tests E8)",
    comando: ["npx", "vitest", "run", "apps/web/tests/spectator.e2e.test.ts", "apps/web/tests/viewer-logic.test.ts", "--maxWorkers=2"],
  ...
  },
  {
    ...
    comando: ["npx", "vitest", "run", "apps/tournament-worker/src/queue.test.ts", "apps/tournament-worker/src/justice.test.ts", "apps/tournament-worker/src/tournament-e2e.test.ts", "--maxWorkers=2"],
    evidencia: "worker matado a mitad de torneo de 20 batallas: reanuda sin duplicar ni perder; commit-reveal de semillas auditable",
    cobertura: "completa",
  },
  {
    id: "replay",
    nombre: "Replay: reproduce el resultado oficial y permite salto temporal",
    comando: ["npx", "vitest", "run", "apps/api/src/e8-replay-verify.test.ts", "apps/replay-service/tests", "apps/web/tests/replay-player.test.ts", "--maxWorkers=2"],
  ...
  },
  {
    ...
    cobertura: "parcial: el `docker compose up` real con healthchecks verdes requiere un host con Docker (checklist en docs/despliegue.md)",
  },
  {
    id: "datos",
    ...
    evidencia: "procedimiento de backup/restore verificado por la suite de E10; migraciones ejecutadas en cada arranque de test-db",
